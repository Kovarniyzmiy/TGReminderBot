...
class tgcalendar(object):
    def __init__(self, day=False, month=False, year=False, parse='', prefix='sel_d_', mprefix='sel_m_', yprefix='sel_y_', TID=''):
        now = datetime.now()
        self.TID = TID
        self.prefix = self.TID + '_' + prefix
        self.mprefix = self.TID + '_' + mprefix
        self.yprefix = self.TID + '_' + yprefix
        self.natext = '✖️'
        self.latext = '⬅️'
        self.ratext = '➡️'
        self.weeknarr = ['Пн', 'Вт', 'Ср', 'Чт', 'Пт', 'Сб', 'Вс']
        ...
        if not parse:
            ...
            self.day, self.month, self.year = parse.split('-')
        if self.month == "13":
    ...
    def create(self):
        ...
        cd = list(calendar.Calendar(firstweekday=0).itermonthdays4(year=self.year, month=self.month))
        ra = []
        ka = [
            [
                {"text": self.latext, "callback": self.yprefix + "1-" + str(self.month) + "-" + str(self.year - 1)},
                {"text": self.year, "callback": self.yprefix + "-"},
                {"text": self.ratext, "callback": self.yprefix + "1-" + str(self.month) + "-" + str(self.year + 1)},
            ],
            [
                {"text": self.latext, "callback": self.mprefix + "1-" + str(self.month - 1) + "-" + str(self.year)},
                {"text": self.month, "callback": self.mprefix + "-"},
                {"text": self.ratext, "callback": self.mprefix + "1-" + str(self.month + 1) + "-" + str(self.year)},
            ]
        ]
        ka.extend([[{"text": x, "callback": self.prefix + '-'} for x in self.weeknarr]])
        ra.extend([] for x in range(int(len(cd)/7)))

        for c in range(len(ra)):
            for k in cd[c*7:(c + 1)*7]:
                if k[1] == self.month:
                    ra[c].append({"text": k[2], "callback": self.prefix + str(k[2]) + "-" + str(k[1]) + "-" + str(k[0])})
                else:
                    ra[c].append({"text": self.natext, "callback": self.prefix + '-'})
        ka += ra
        return ka
...
# c = tgcalendar().create()
# print(json.dumps(c.create(), indent=2))
# for i in c.create():
#     print(i)
# print(c)
# print(list(calendar.Calendar().itermonthdays4(year=2022, month=12)))
